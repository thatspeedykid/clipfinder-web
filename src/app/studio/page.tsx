    ...
    <main className="min-h-screen flex flex-col bg-[#0f0f0f] text-white">
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-10">
        <div className="mb-8">
          ...
          {quota && <p className={`text-xs mt-2 ${atLimit ? 'text-red-400' : 'text-white/30'}`}>{quota.used}/{quota.limit} generations used today</p>}
        </div>

        {atLimit && (
          <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4 mb-6 flex items-center justify-between gap-4">
            <p className="text-red-400 text-sm">Daily limit reached. {profile?.tier === 'free' ? 'Upgrade for 25/day.' : 'Resets in 24 hours.'}</p>
            {profile?.tier === 'free' && <Link href="/pricing" className="text-xs bg-[#FF6B00] text-white px-3 py-1.5 rounded-lg">Upgrade</Link>}
          </div>
        )}
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
            ...
            {inputType === 'url' ? (
              <input type="url" value={input} onChange={e => setInput(e.target.value)} required
                placeholder="https://youtube.com/watch?v=... or kick.com/clip/..."
              ...
              <textarea value={input} onChange={e => setInput(e.target.value)} required
                placeholder={inputType === 'transcript' ? 'Paste the transcript or subtitles here...' : 'Describe what happened in the clip. Names, what was said, the drama...'}
                rows={inputType === 'transcript' ? 8 : 4}
          ...
          <button type="submit" disabled={generating || !input.trim() || !!atLimit}
            className="w-full py-3 bg-[#FF6B00] text-white font-medium rounded-2xl hover:bg-[#e55f00] disabled:opacity-50 text-sm">
            {generating ? (inputType === 'url' ? '🔍 Extracting transcript...' : '✨ Generating 3 posts...') : '✨ Generate posts'}
          </button>
        </form>

        {error && <div className="mt-4 bg-red-500/10 border border-red-500/20 rounded-2xl p-4"><p className="text-red-400 text-sm">{error}</p></div>}
        ...
        {result && (
          <div className="mt-8 space-y-4">
            ...
            {result.extracted_title && <p className="text-xs text-white/30 px-1">Detected: {result.extracted_title}</p>}
            ...
            <button onClick={() => setResult(null)} className="w-full text-xs py-2.5 rounded-2xl bg-white/5 text-white/30 border border-white/10 hover:bg-white/10">↺ Generate new posts</button>
          </div>
        )}
      </div>
    </main>
