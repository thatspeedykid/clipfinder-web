...
async function r2Delete(storagePath: string): Promise<boolean> {
  ...
  if (!accountId || !accessKeyId || !secretAccessKey || !storagePath) return false
  try {
    const now = new Date()
    ...
    const authorization = `AWS4-HMAC-SHA256 Credential=${accessKeyId}/${credentialScope}, SignedHeaders=host;x-amz-date, Signature=${signature}`
