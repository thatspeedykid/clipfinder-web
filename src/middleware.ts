...
// Handles: maintenance mode, BRB mode, sandbox mode, IP blocking, VPN detection, banned users
...
// Public paths that always work even in maintenance mode
...
export async function middleware(req: NextRequest) {
  ...
  // Always allow static and auth paths
  ...
  // Fetch flags + blocked IPs from Supabase in parallel
  ...
  if (flagsRes.status === 'fulfilled' && Array.isArray(flagsRes.value)) {
    flagsRes.value.forEach((f: { key: string; enabled: boolean }) => {
      flags[f.key] = f.enabled
    })
  }

  const isIpBlocked = blockedIpRes.status === 'fulfilled' &&
    Array.isArray(blockedIpRes.value) &&
    blockedIpRes.value.length > 0
  ...
  if (flags.feature_vpn_block && ip !== 'unknown') {
    ...
    if (vpnApiKey) {
      ...
      } catch { /* VPN check failed silently — don't block on error */ }
    }
  }
  ...
  try {
    ...
    if (user) {
      ...
      // ── Banned user check ────────────────────────────────────────────────
      if (profile?.is_banned && !ADMIN_PATHS.some(p => pathname.startsWith(p))) {
        return new NextResponse(
          blockedPage('Account Banned', profile.ban_reason ?? 'Your account has been banned.'),
  ...
  // ── Sandbox mode (admin only access) ──────────────────────────────────────
  if (flags.site_sandbox_mode && !pathname.startsWith('/api/auth')) {
    return new NextResponse(
